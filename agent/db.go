...
func InitDB(path string) error {
	...
	if _, err = db.Exec(`ALTER TABLE shipments ADD COLUMN seller TEXT NOT NULL DEFAULT ''`); err != nil && !strings.Contains(err.Error(), "duplicate column name") {
		return err
	}
...
// SeedDemoDataIfEmpty seeds the demo vendors and products on first boot (or after
// an ephemeral-disk reset on Render). Safe to call on every startup — it only
// writes when the vendors table is empty.
func SeedDemoDataIfEmpty() {
	var count int
	if err := db.QueryRow(`SELECT COUNT(*) FROM vendors`).Scan(&count); err != nil || count > 0 {
		return
	}

	vendors := []struct{ wallet, shop, desc, vtype, cats string }{
		...
		{"GLOBAL_COLD_CHAIN", "Global Cold-Chain Systems", "Specialised logistics hardware for temperature-controlled shipping.", "distributor", "Cold Chain"},
	}
	for _, v := range vendors {
		db.Exec(`INSERT OR IGNORE INTO vendors (wallet, shop_name, description, vendor_type, categories) VALUES (?,?,?,?,?)`,
	...
	products := []struct {
		...
		{"LOGOS_IOT_SOLUTIONS", "Smart Mesh Gateway Hub", "Centralised connectivity for enterprise IoT mesh networks.", "IoT Connectivity Hub", "IoT Hardware", "https://images.unsplash.com/photo-1544197150-b99a580bb7a8?w=800&q=80", "wholesaler", 299.00, 4.5, 10, 7},
	...
	for _, p := range products {
		db.Exec(`INSERT INTO products (vendor_wallet, title, description, short_description, price_usdc, category, image_url, in_stock, moq, lead_time_days, rating, seller_tier) VALUES (?,?,?,?,?,?,?,true,?,?,?,?)`,
